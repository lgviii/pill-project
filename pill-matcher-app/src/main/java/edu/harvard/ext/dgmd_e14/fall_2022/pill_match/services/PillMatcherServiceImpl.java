...
public class PillMatcherServiceImpl implements PillMatcherService {
    ...
    /**
     * Reduces the specified model output map to just the top MODEL_OUTPUT_LIMIT classes (ordered on probability), for
     * use in database queries.
     *
     * @param modelOutputMap Map containing feature class linked to probability, as generated by the model
     * @return List with just the top MODEL_OUTPUT_LIMIT classes from the specified map
     */
    ...
    /**
     * Reduces the specified map containing all possible matched pills to just the top PILL_MATCH_LIMIT matches.  If
     * the map contains fewer than the limit, returns the entire map.
     *
     * @param pillMatchMap Map of all possible pill matches
     * @return Map containing only the top PILL_MATCH_LIMIT pill matches in order by overall probability (highest to
     *         lowest)
     */
    ...
    /**
     * Combines the predicted imprints from the specified outputs into a single prediction String - assumption is
     * that the model outputs are from different images that belong to the same pill (front + back), so any predicted
     * imprints should be combined to better match the actual pill imprint.
     *
     * @param imageModelOutputs Collection containing multiple model outputs from different images assumed to belong
     *                          to the same pill
     * @return List of combined imprints from all images
     */
